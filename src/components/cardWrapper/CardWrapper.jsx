...
const Card = ({cloudinaryImageId, name, avgRating, cuisines, locality, costForTwo}) => {
  ...
  return(
    <div className='res-card'>
      ...
      <div className="res-details">
        ...
        <p className='res-loc'>{locality}</p>
      </div>
    </div>
  )
...
const CardWrapper = ({resDetails}) => { 
  ...
  return resDetails?.length === 0 ? <ShimmerSimpleGallery card row={2} col={4} imageType="thumbnail" gap={20} imageHeight={248} caption /> : (
    <div className='card-wrapper'>
        {resDetails?.map((item, index) => <Card key={index} {...item?.info}/>)}
    </div>
  )
}
